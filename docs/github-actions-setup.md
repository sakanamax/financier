...
- 2026-06-08：新增 legacy repository 後，dependency 解析階段已通過，但 build 失敗於 `:app:checkUntiedDebugDuplicateClasses`。原因是 `com.google.common.util.concurrent.ListenableFuture` 同時存在於 `com.google.guava:guava:20.0` 與 `com.google.guava:listenablefuture:1.0`；因此全域排除 `com.google.guava:listenablefuture`，保留既有 Guava。
- 2026-06-08：duplicate class 修正後，build 進一步失敗於 `:app:kaptUntiedDebugKotlin`，原因是 `untied` flavor 沒有讀取 `Export`、`ImportExportAsyncTask` 與 Google Drive 相關來源。因 `untied` 目標是自用 cloud sync 測試 APK，因此讓 `untied` source set 讀取 `src/googleplay/java` 與 `src/googleplay/res`。
- 2026-06-08：source set 修正後，build 進一步失敗於 `:app:compileUntiedDebugKotlin`。問題集中在 AndroidX Preference API 的 non-null override 簽名與 Fragment nullable context/dialog 使用；已做 Kotlin 相容性修正。
- 2026-06-08：run `27146328437` 成功完成 `Build Debug APK` 與 `Upload Debug APK Artifact`，並確認 artifact `financier-debug-apks` 可下載。本地解壓後 APK 路徑為 `untied/debug/Financier.apk`。

## 結案狀態
- 狀態：部分完成；Actions artifact 已可產出，尚待實機驗證與 Release 決策
- 完成日期：尚未完成
- 結案說明：GitHub Actions 已可建置並上傳 `untiedDebug` APK；下一步是安裝到實機驗證 Google Drive / Dropbox 同步修復。
